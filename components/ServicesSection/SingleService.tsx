...
// Animációs variáns a balról nyíló scaleX-hez
    ...
    <div className="flex w-full flex-col">
      {myServices.map((service) => (
        <motion.div
          ...
          className=" flex min-h-screen w-full flex-col justify-center bg-dark-color"
        >
          <div className="mx-auto max-w-5xl">
            <span className="text-sm font-semibold uppercase tracking-wider text-green-500">
              0{service.id + 1} // Szolgáltatás
            </span>
            <h2 className="mt-2 text-3xl font-bold md:text-5xl lg:text-6xl">
              {service.title}
            </h2>
            ...
            <ul className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {service.benefits.map((benefit, idx) => (
                ...
                >
                  <span className="mt-1 flex h-2 w-2 rounded-full bg-green-500" />
                  <span>{benefit}</span>
                </li>
              ))}
            </ul>
